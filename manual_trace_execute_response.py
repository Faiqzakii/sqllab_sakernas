...
def main() -> None:
    ...
    try:
        ...
        def capture_response(response) -> None:
            ...
            if "/api/v1/sqllab/execute/" not in str(url) and "/api/v1/sqllab/results/" not in str(url):
                return
            try:
                payload = response.json()
            except Exception as exc:  # noqa: BLE001
                payload = {"json_error": str(exc)}
            captured.append({"url": str(url), "payload": payload})
        ...
        print(json.dumps({"captured": captured, "current_url": page.url}, ensure_ascii=False, indent=2))
    finally:
        auth_result.close()
